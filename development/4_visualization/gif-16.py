...
file_path = "5_mid-examples/1_bth-str-trio.mid"
...
# frequence and amplitude of plot 
freq = ticks_per_beat / 100
ampl = 3 

thirty_second_ticks = file_mid.ticks_per_beat // 16

thirty_second_sec = int(mido.tempo2bpm(thirty_second_ticks)/960)
print(thirty_second_sec)
...
for i, track in enumerate(file_mid.tracks):
    ...
    for msg in track:
        ...
        if count_msg_1 == 0:
            if msg.time != 0:
                bpm = mido.tempo2bpm(msg.time)
                beats_per_second = bpm * 60
                sixteenth_per_second = beats_per_second * 4
                count_msg_1 = 1
                print(sixteenth_per_second)
        """
...
def schritt(i):
    ...
    # thickness and pitch per instr -> line per instr
    for instr in all_pitches_d_m:
        # new thickness
        thickness_list = all_velocities_d[count_instr]
        thickness = thickness_list[count_thickness_i]
        ...
        # new pitch list
        pitch_100 = [] # list with new pitches
        pitch_list = all_pitches_d_m[count_instr] # all pitches from instr
        count_pitches_100 = count_thickness_i # index of new pitch list
...
animation = FuncAnimation(abbildung, schritt, frames = 1000, interval = 20, blit = False)

animation.save('bth-str-trio.gif')
