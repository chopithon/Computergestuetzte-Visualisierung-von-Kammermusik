...
file_path = "5_mid-examples/7_moz-qit.mid"
...
# Bestimmen, wie oft die Tonhöhe angehängt werden soll 
# (in Bezug auf die Tondauer) 
def get_pitch_factor(duration):
	pitch_factor = int(duration//thirty_second_ticks)
	return pitch_factor

# Dicke der Linie bestimmen (in Bezug auf die Lautstärke des Tones)
...
# --- Informationen über die Linie (Tonhöhe, Tondauer, Lautstärke, Takterkennung) --- #
for i, track in enumerate(file_mid.tracks):
    ...
    for msg in track:
        ...
        # Liste mit Tonhöhe und Liste mit Lautstärke 
		# jedes Instruments erstellen (inkl. Tondauer)
        if msg.type == "note_on" or msg.type == "note_off":
            ...
            if msg.time != 0:
                ...
                # Tonhöhe (inkl. Tondauer) an Liste anhängen
    ...
    # Liste jedes Instruments an Liste mit allen Instrumenten anhängen 
...
# Liste mit der Tonhöhe und Lautstärke jedes Instruments an neue Liste anhängen
# Nur das erste Instrument erkennt noch die Takte, die anderen nicht mehr
count_instruments = 0
...
print("--- Informationen für die Linie completed ---")

# --- Informationen zur Ebene (Harmonie) --- #

# Jeder Tonart ihre dazugehörige Farbe bestimmen
...
print("--- Informationen zur Ebene completed ---")
...
for instr in all_pitches_d_m:
    ...
    if count_instr == 0:
        for i in listed:
            ...
            index += 1
    count_instr = 1
count_color_i = color_i[0]

# --- Visualisierung --- #
# Basis Komponente der Linie
...
# Achsen regulieren (Jede Achse gleich lang)
